...
#running as root gives the wrong homedir, check and exit if run with .
if ((EUID == 0)); then
    echo "The script is not designed to run as root user. Please run it without  prefix."
    exit
fi
...
chmod 644 avalanchego.service
 cp -f avalanchego.service /etc/systemd/system/avalanchego.service
 systemctl start avalanchego
...
echo " systemctl status avalanchego"
